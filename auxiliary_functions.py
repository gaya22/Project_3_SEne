...
'''This function gives back a tuple that has the only purpose to fill the table
    with the peak of fraction of electricity generated by gas'''
def get_pick_gas(df):
...
def considerations():
    ...
    mystr += "Electricity exports and electricity available to the market (that is to say: electricity surplus) are not that well correlated to the metric of our interest. "
    mystr += "The electricity generated shows some peaks in correspondance to the gas imports, but most of it looks delayed by some months. "
    mystr += "The comparison between gas imports and gas consumption shows that when Italy consumes lots of gas, it also imports a lot of gas (peaks) if it is winter, as it can also be noted with a relevant negative correlation value between the average temperature in the country and and the gas consumned. "
    ...
    mystr += "On the other hand, the amount of gas produced and the amount of gas exported are so little that they do not have any impact on the rest of the metrics. "
    mystr += "The data about population does not seem to be well correlated to anything else. "
    ...
    mystr += "The correlation that relates the most with the goal of our project is about gas prices, that are clearly tied with the imports from certain countries, mainly due to some historical events that happened through the last years. "
